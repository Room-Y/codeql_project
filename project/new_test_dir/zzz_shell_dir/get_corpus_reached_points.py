...
def get_thread_result(binfile, filter_binfile, crash_sublist):
    ...
    for idx, crash in enumerate(crash_sublist):
        ...
        # if(True or p.returncode == None or p.returncode != 0):
        #     sub_crash_num += 1
        #     try:
...
def get_crash_points(binfile, filter_binfile, crashdir, jsondir):
    ...
    print("len_listjson:", len(list_json))
...
if __name__ == "__main__":
    ...
    numfuzz = ["1", "2", "3"]
    corpusname = ["_fuzz/out", "_fuzz/out/default/queue", "_fuzz/out/queue", "_fuzz/out"]
    # corpusname = ["_fuzz/out"]
    ...
    for idx, fuzzn in enumerate(fuzzname):
        ...
        for numf in numfuzz:
            ...
            for proj in list_proj:
                ...
                corpusdir = root_dir + "remove_" + fuzzname_num + corpusname[idx]
                jsondir = root_dir + "new_json_remove_" + proj
                dict, ids = get_crash_points(binfile, filter_binfile, corpusdir, jsondir)
            ...
            csv.to_csv("zzz_csv_dir/corpus1_" + fuzzname_num + "_points.csv")
